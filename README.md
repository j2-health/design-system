...
2. **Styles** — import the compiled component styles once (e.g. in your app
   ...
   With Tailwind 3, **this file is your Tailwind entry point.** It supplies
   `@tailwind base`, `@tailwind components`, and `@tailwind utilities` along
   with the design tokens and base element styles, so remove those directives
   from your own stylesheet — keeping both emits every Tailwind layer twice.

   With Tailwind 4, import Tailwind before the design-system stylesheet and load
   your JavaScript config explicitly:
   ...
   4 no longer discovers JavaScript config files automatically.
