...
other, or when a card is already distinguished by a selected border. The lighter
shadow keeps the hover cue legible without competing with borders or pulling
focus. Dropping the border on hover additionally made the card appear to
collapse/shift, which was the other half of the visual noise people were
reacting to.
...
- If we later need a stronger lift for a specific surface (e.g. a standalone
  marketing card), consider a `shadow="lg"` prop rather than rolling the global
  value back up.
